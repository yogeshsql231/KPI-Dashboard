...
/**
 * ETL: pull A/R invoices + payment-received dates from a source system
 * (PRODHANA / PRIMSBM) and upsert them into the local MySQL `ar_payments`
 * cache that feeds the "Late Deliveries vs Late Payments" report.
 *
 * Run on the local server (XAMPP box) where the source DBs are reachable:
 *
 *   php etl/pull_payments.php --source=PRODHANA
 *   php etl/pull_payments.php --source=PRIMSBM --query=etl/queries/prodhana_payments.sql --via=PRODHANA
 *
 * --via=<LinkedServer>: run the (HANA) query THROUGH a SQL Server linked server
 * using OPENQUERY (same mechanism as pull_delivery.php).
 *
 * The source query must return these output columns (alias them in the .sql
 * file to match your real schema):
 *   source_key, invoice_num, customer_code, customer_name,
...
$queryFile = (string) ($opts['query'] ?? (__DIR__ . '/queries/' . strtolower($source) . '_payments.sql'));
...
$textCols = ['invoice_num', 'customer_code', 'customer_name'];
/** Date columns normalised to YYYY-MM-DD (paid_date may be NULL = still open). */
$dateCols = ['invoice_date', 'due_date', 'paid_date'];
/** Numeric columns. */
$numCols = ['invoice_amount', 'paid_amount'];
...
$upsertSql = "INSERT INTO ar_payments ($colList)
   VALUES ($placeholders)
...
try {
    while (($row = $stmt->fetch(PDO::FETCH_ASSOC)) !== false) {
        ...
        // HANA folds unquoted aliases to UPPERCASE; lowercase the keys so the
        // matching is robust across sources.
