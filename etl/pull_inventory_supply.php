...
/**
 * ETL: pull on-hand + trailing usage per item x warehouse from a source system
 * into the local MySQL `inventory_supply` cache that feeds the Procurement
 * dashboard's Inventory Days of Supply widget (SCRUM-92).
 *
 * Run on the local server (XAMPP box) where the source DBs are reachable:
 *
 *   php etl/pull_inventory_supply.php --source=PRIMSBM --query=etl/queries/prodhana_inventory_supply.sql --via=PRODHANA
 *   php etl/pull_inventory_supply.php --source=PRIMSBM --query=etl/queries/prodhana_inventory_supply.sql --via=PRODHANA --dry-run
 *
...
$queryFile = (string) ($opts['query'] ?? (__DIR__ . '/queries/' . strtolower($source) . '_inventory_supply.sql'));
...
$flagCols = ['is_active'];
$expected = array_merge(['source_key'], $textCols, $dateCols, $numCols, $flagCols);
$allCols  = array_merge($textCols, $dateCols, $numCols, $flagCols);
...
$upsertSql = "INSERT INTO inventory_supply ($colList)
   VALUES ($placeholders)
