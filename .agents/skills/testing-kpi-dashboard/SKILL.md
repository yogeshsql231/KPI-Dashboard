...
## View-migration gotcha (applies to any new filter/column)
- MySQL views (`vw_order_shipment_kpi`, `vw_delivery_lines`, ...) have FIXED SELECT lists frozen at creation. If a page filter references a column added by a later migration (e.g. `so_docentry`, `warehouse` from migration 002), the whole page can crash with the generic "Unable to load KPI data" banner while the unfiltered page works fine.
- Debug pattern: instantiate the repository in `php -r` and call each method with the failing Filters to get the real PDO exception, instead of guessing from the generic banner.
- Fix pattern: add a `CREATE OR REPLACE VIEW` migration (see `sql/migrations/011_view_so_docentry.sql`). Do NOT add the columns to the base view in `sql/schema.sql` — fresh installs run schema.sql before migration 002, so the view would reference missing columns.
- Always remind the user to run new migrations on the XAMPP box after `git pull`; pages may keep crashing there otherwise.

## Customer Service dashboard (SCRUM-66/67) specifics
- Order Status Tracking panel reads `delivery_lines` grouped by `so_status` via `KpiRepository::orderStatusTracking()` + `Filters::deliveryClause()`. DB-verify with: `SELECT COALESCE(NULLIF(so_status,''),'Unknown'), COUNT(DISTINCT sales_order), COUNT(DISTINCT po_number), COUNT(*), SUM(order_qty), SUM(released_qty), SUM(delivered_qty) FROM delivery_lines GROUP BY 1`.
- The CS filter bar fields sit close together (SO number vs PO number text inputs) — after typing, confirm via the DOM which input actually got the text before clicking Apply.
- The CS warehouse dropdown may only show "All warehouses" if seeded `order_shipments.warehouse` is empty; verify warehouse filtering via a `?warehouse=...` HTTP request in that case and note it in the report.
