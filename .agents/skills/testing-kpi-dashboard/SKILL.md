...
## Pallet widget (SCRUM-105, reverted to consolidated by SCRUM-106) specifics
- `lpn_pallets` keeps `item_type`/`pallet_value` columns (migration 021 + LPN pull), but the Overview widget no longer displays a raw/FG split — it shows plain stacked status bars per group plus a bottom "Consolidated: N pallets per the applied filters" line. N must equal the sum of the group totals under the active warehouse/item filters.
- To test: seed temp rows with `source_system='SAMPLE-TYPE'` covering Newark/CLIFFTON/Clifton-MissingLPN/FG-BROOKLYN, then DELETE by source_system and confirm COUNT=0.
...
- item_type classification is heuristic (OITB group name %raw%/%ingredient% vs %finish%/%fg% or FG-prefixed warehouse); it still feeds the ETL even though the UI no longer shows the split.

## Overview redesign (SCRUM-82, reworked by SCRUM-102) specifics
- Overview has client-side behaviors driven by `DATA` JSON in `public/overview.php` + `public/assets/overview.css`: correlation month drill-down and drag-to-reorder sections (KPI tile hover sparklines were removed by SCRUM-106 — don't expect graphs on hover). Since SCRUM-102 the gold "executive" theme and the pallet Loaves/Bars toggle are gone: Overview uses the shared style.css palette (navy topbar, blue accents) and pallets-by-warehouse renders plain stacked `.pbar` bars with per-status tooltips; the Pallets & Orders grid is `g5` (no Cycle time tile — don't expect one).
...
- Known quirk: `resetLayout()` hardcodes section ids and may omit newer sections (e.g. `araging`), so a newer section can jump to the top right after clicking "reset layout"; a reload restores DOM default order. If observed, check whether the id list matches the `data-id` attributes before flagging.
- HTML5 drag-reorder works with a plain `left_click_drag` from a ⠿ section handle to above another section label; watch for the "saved" flash in the filter bar. With the computer tool, `left_mouse_down` does not take a coordinate — `mouse_move` to the handle first, then `left_mouse_down`, incremental `mouse_move`s, `left_mouse_up`.
- A/R aging (open invoices) widget: the shipped seed usually has NO open invoices (all rows paid), so the section shows its empty state. To test buckets, insert temp `ar_payments` rows with `paid_date NULL` and `source_system='SAMPLE-OPEN'` spanning each bucket (include a partial payment: open = invoice_amount − paid_amount), verify against a GROUP BY on `DATEDIFF(CURDATE(), due_date)`, then `DELETE FROM ar_payments WHERE source_system='SAMPLE-OPEN'`. Since SCRUM-106 the widget honors the page filters: `invoice_date` within the date range, and the warehouse filter matches customers with `delivery_lines` orders in that warehouse group (invoices carry no warehouse); buckets still age from CURDATE().
...
- The "Estimated vs Actual Production Usage" panel lives on `warehouse.php` (moved off Delivery — its absence there is expected, but Delivery's Stock Stage Tracking must remain). Backed by `WarehouseInventoryRepository::productionUsageSummary/ByGroup/ByOrder/ByItem` over `production_usage` (migration 012, Beas `@BMM_PNMAST`/`@BMM_PNITEM` via `etl/queries/prodhana_production_beas.sql`).
